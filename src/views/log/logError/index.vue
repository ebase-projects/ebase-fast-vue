...
<script>
import { listLogError } from '@/api/log/logError'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listLogError(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
        if (response.code === 0) {
          this.logErrorList = response.data.list
